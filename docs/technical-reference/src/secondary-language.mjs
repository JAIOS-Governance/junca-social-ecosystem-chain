...
export const secondaryTranslations = [
  ...
  ["外部パートナーが発行・運用する資産の経済的取扱いと法的分類はプロトコル検証と分離し、管轄法に応じて確認します。", "外部合作伙伴发行或运营资产的经济处理和法律分类与协议验证分开，并按适用司法管辖区审查。", "El tratamiento económico y la clasificación jurídica de los activos de socios externos se separan de la validación del protocolo y se revisan según la jurisdicción aplicable.", "Il trattamento economico e la classificazione giuridica degli asset di partner esterni sono separati dalla validazione del protocollo e verificati secondo la giurisdizione applicabile.", "تُفصل المعالجة الاقتصادية والتصنيف القانوني لأصول الشركاء الخارجيين عن التحقق من البروتوكول وتُراجع وفق الولاية القضائية المطبقة."],
