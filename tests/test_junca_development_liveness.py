    ...
    def __init__(
        ...
        *,
        ...
        pending = (
            ...
            else type("Proposal", (), {"block_timestamp": pending_timestamp})()
        )
...
class DevelopmentRecoveringBoundedFinalityLoopTests(unittest.TestCase):
    def make_loop(self, state: _State):
        return DevelopmentRecoveringBoundedFinalityLoop(
            state,
            interval_seconds=30,
    ...
    def test_partial_failure_is_not_replayed_in_same_slot(self) -> None:
        state = _State(1_800_000_030, fail_broadcasts=1)
        loop = self.make_loop(state)

        with self.assertRaisesRegex(ValidatorNodeError, "peer vote delivery failed"):
    ...
    def test_new_proposal_uses_current_slot(self) -> None:
        state = _State(None)
        loop = self.make_loop(state)
        self.assertTrue(loop.run_once(1_800_000_060))
    ...
    def test_future_pending_timestamp_fails_closed(self) -> None:
        ...
        loop = self.make_loop(state)
        with self.assertRaisesRegex(ValidatorNodeError, "ahead of the canonical slot"):
            loop.run_once(1_800_000_060)
