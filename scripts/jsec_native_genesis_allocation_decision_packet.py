#!/usr/bin/env python3
"""Emit the exact CEO decision packet for JSEC native Genesis allocations."""
...
def main() -> int:
    ...
    try:
        packet = load_native_token_genesis_plan(
            args.plan
        ).allocation_decision_packet()
    except NativeTokenGenesisError as exc:
        print(f"JSEC Genesis allocation packet failed: {exc}", file=sys.stderr)
        return 1
    ...
    if args.require_approved and packet["status"] != "approved":
        print("JSEC Genesis allocation approval is still required", file=sys.stderr)
        return 2
    return 0
