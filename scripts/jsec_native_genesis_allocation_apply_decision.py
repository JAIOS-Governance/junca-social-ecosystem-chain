#!/usr/bin/env python3
"""Apply a verified CEO allocation decision to a non-activated JSEC plan."""
...
from jaios.social_ecosystem_chain.native_token_genesis import (
    ...
    load_native_genesis_allocation_decision,
)
...
def main() -> int:
    ...
    if len(set(resolved.values())) != len(resolved):
        print(
            "JSEC allocation decision input and output paths must be distinct",
            file=sys.stderr,
        )
        return 2
    ...
    try:
        ...
        decision = load_native_genesis_allocation_decision(args.decision)
        updated = apply_native_genesis_allocation_decision(
            raw_plan,
            decision,
        )
        ...
        print(
            f"JSEC Genesis allocation decision apply failed: {exc}",
            file=sys.stderr,
        )
        return 1
