#!/usr/bin/env python3
"""Apply a verified CEO decision record to a non-activated JSEC Genesis plan."""
...
def main() -> int:
    ...
    if args.output.resolve() == args.evidence_output.resolve():
        print(
            "JSEC native economics output paths must be distinct",
            file=sys.stderr,
        )
        return 2
    ...
    try:
        ...
        decision = load_native_economics_decision(args.decision)
        updated = apply_native_economics_decision(raw_plan, decision)
        output_plan = evaluate_native_token_genesis_plan(updated)
    except NativeTokenGenesisError as exc:
        print(f"JSEC native economics decision apply failed: {exc}", file=sys.stderr)
        return 1
