#!/usr/bin/env python3
"""Emit the exact CEO decision packet required to lock JSEC native economics."""
...
def main() -> int:
    ...
    try:
        ...
        print(f"JSEC native economics packet failed: {exc}", file=sys.stderr)
        return 1
    ...
    if args.require_approved and packet["status"] != "approved":
        print("JSEC native economics approval is still required", file=sys.stderr)
        return 2
    return 0
